	...
	private void validate(int numberCount, Range range) {
		...
		if (numberCount <= 0) {
			throw new IllegalArgumentException(
				String.format("numberCount must be greater than 0, but provided %d", numberCount));
		}
		if (range.size() < numberCount) {
			throw new IllegalArgumentException(
				String.format("Range size(%d) is too small than numberCount(%d)", range.size(), numberCount));
		}
	}
}
